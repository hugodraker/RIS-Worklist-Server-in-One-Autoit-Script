...
szConfigFmt         db "Config: AET=%s DicomPort=%u DebugLog=%u LastFileIndex=%u",13,10,0
...
; FIX: flat filename format, no SR subfolder
szStoragePathFmt    db "%06u.DCM",0
szUIntFmt           db "%u",0
...
; ==============================================================================
; INI / CONFIG
; FIX: only write defaults on first run (when INI file doesn't exist).
;      Otherwise just READ values and leave the INI alone, so LastFileIndex
;      that was saved by previous runs is preserved.
; ==============================================================================
...
LoadConfig PROC
    ...
    ; Check if the INI file exists. If not, this is first-run -> seed defaults.
    invoke GetFileAttributes, OFFSET szIniFile
    mov fAttr, eax
    cmp eax, INVALID_FILE_ATTRIBUTES
    jne LC_HaveIni

    ; First run: write all default keys exactly once.
...
LC_HaveIni:
    ; Now read whatever is in the INI (the values we may have just written,
    ; or pre-existing user-edited values).
...
; ==============================================================================
; STORAGE PATH
; FIX: flat numbering. Filename is "000001.DCM", "000002.DCM" ... in current
;      directory. LastFileIndex starts from whatever LoadConfig read out of INI.
;      Each image: increment, persist to INI, format path.
; ==============================================================================
GetNextStoragePath PROC pOut:DWORD
    ...
    invoke crt_sprintf, pOut, OFFSET szStoragePathFmt, g_CurrentFileIndex
    ret
...
; ==============================================================================
; A-ASSOCIATE-AC BUILDER HELPERS  (unchanged from previous working build)
; ==============================================================================
